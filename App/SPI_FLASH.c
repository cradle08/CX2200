...
/*宏定义区******************************************************************/
#define FLASH_CS_HIGH									FLASH_SPI_CS_GPIO_Port->BSRR |= FLASH_SPI_CS_Pin
#define FLASH_CS_LOW									FLASH_SPI_CS_GPIO_Port->BSRR |= (uint32_t)FLASH_SPI_CS_Pin<<16
...
uint8_t SPI4ModeFlag = 0;						//用于表示SPI4当前处于什么模式状态，为0表示处于与XPT2046通讯的状态，为1表示处于与SPI FLASH通讯状态，为2表示处于与AD数字电位器通讯状态
...
***/
void SPI4_ChangeModeToXPT2046(void)
{
	extern SPI_HandleTypeDef hspi4;
	extern uint8_t SPI4ModeFlag;
...
/***
*切换SPI4模式与AD数字电位器进行通讯：分频256，模式：0，0
***/
void SPI4_ChangeModeToAD(void)
{
	extern SPI_HandleTypeDef hspi4;
	extern uint8_t SPI4ModeFlag;
