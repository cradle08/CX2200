...
void NAND_EraseChip(void)
{
    uint8_t status;
    uint16_t i=0;
    for(i=0;i<NAND_BLOCK_NUM;i++) //循环擦除所有的块
    {
        status=NAND_EraseBlock(i);
        if(status)LOG_Info("Erase %d block fail!!，错误码为%d",i,status);//擦除失败
    }
}
...
void NAND_EraseChip_All(void)
{
    ...
    for(i=0; i<nand_dev.block_totalnum; i++)
    {
        ...
        if(temp)LOG_Info("write spare 1 or 2 in block fail!!，错误码为%d",i);//擦除失败
