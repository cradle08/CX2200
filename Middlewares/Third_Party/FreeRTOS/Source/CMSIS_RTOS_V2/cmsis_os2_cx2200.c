...
{
  extern  uint32_t pcTaskGetStackAddr( TaskHandle_t xTaskToQuery );
  TaskHandle_t hTask = (TaskHandle_t)thread_id;
  ...
  if (IS_IRQ() || (hTask == NULL)) {
    ...
    ulStackSize = pcTaskGetStackAddr (hTask);
  }
...
{
  extern  uint32_t pcTaskGetStackSize( TaskHandle_t xTaskToQuery );
  TaskHandle_t hTask = (TaskHandle_t)thread_id;
  uint32_t ulStackSize = 0;
  ...
  if (IS_IRQ() || (hTask == NULL)) {
    ...
    ulStackSize = pcTaskGetStackSize (hTask);
  }
