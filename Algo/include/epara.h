...
// 取参数
ePara * getParaPtrEP(eParaList *paraList, const char *name);

// 追加参数（若参数名重复，覆盖原参数）
BOOL8 appendDetailParaEP(eParaList *paraList, const char *name, uint8 dataType, uint32 dataNum, 
						const char *dataArray, uint8 flag, const char *unit);
BOOL8 appendParaEP(eParaList *paraList, const ePara *para);
